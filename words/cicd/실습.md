...
- 개발 프로세스의 자동화는 외부툴 없이 자동화 프로세스를 구축 할 수 있다.

- pr을 하기전에 테스트를 할 수 있는 텍스트 파일도 작성을 해주어야 한다.

- github actions는 Event를 지정할 수 있고 커밋을 푸쉬하고 새로운 이슈를 누군가가 만들때 이에 대해서 이벤트를 지정할 수 있다.

- workflow는 event에 대해서 그 이벤트에 대한 워크플로우를 작성할 수 있다. 

- job은 workflow 안에 job이 있고 이에 대해서 job안에는 steps로 할 수도 있고 shell script 사용가능하고 actions는 github에서 제공하는 서비스를 action을 이용해서 처리할 수 있다.

- 잘 만들어진 action을 잘 이용하는 것도 매우 중요하다. 

- job을 실행하게 해주는 것은 바로 runner라고 할 수 있다.

- push가 이루어질때 마다 정할 수 있다는 점이다. 

- steps에서는 순서에 따라서 실행되도록 할 수 있게 해준다. 

- branch를 지정해서 해당 branch에 실행이 될때 이를 사용할 수 있게 해줄 수 있다.

- 보통 push, pr을 할때 CI가 작동하도록 하고 CD는 main, develope에 경우 작동할 수 있도록 한다. 

- pull_request는 내가 pr을 보내거나 또는 다운을 받을때 이에 대해서 확인을 할 수 있다.

- CI를 거치고 Docker build를 이용해서 registry를 만들고 deploy를 하는 흐름이라고 생각하면된다. 